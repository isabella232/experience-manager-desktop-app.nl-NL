---
title: Desktop app versie 1.x installeren en [!DNL Experience Manager] configureren
description: Installeer en [!DNL Experience Manager] desktop app version 1.x to work with [!DNL Assets] configureer reservers en wijs de middelen toe om als aandrijving op uw Desktop te monteren.
...
# Desktop app v1.x installeren en configureren [!DNL Experience Manager] {#install-and-configure-aem-desktop-app}

Met de [!DNL Experience Manager] [!DNL Experience Manager] bureaubladtoepassing zijn de elementen in de app gemakkelijk toegankelijk op uw lokale bureaublad en kunnen deze worden gebruikt in alle bureaubladtoepassingen. Elementen kunnen eenvoudig worden weergegeven in Mac Finder of Windows Verkenner, worden geopend in bureaubladtoepassingen en lokaal worden gewijzigd. De wijzigingen worden weer opgeslagen [!DNL Experience Manager] wanneer u uploadt en er wordt een nieuwe versie gemaakt in de opslagplaats.
...
Als u de [!DNL Experience Manager] bureaubladtoepassing wilt gebruiken,

* Zorg ervoor dat uw [!DNL Experience Manager] serverversie wordt ondersteund door de [!DNL Experience Manager] bureaubladtoepassing. Zie de [compatibiliteitsmatrix](release-notes-of-v1.md#compatibilitymatrix).
...
Zie de opmerkingen bij de release van de [[!DNL Experience Manager] bureaubladtoepassing](release-notes-of-v1.md)voor meer informatie.
...
Zie [Installeren en [!DNL Experience Manager] desktop app to [!DNL Experience Manager] verbinden met server](use-app-v1.md#installandconnect)voor meer informatie.
...
Voor de kopieer- en verplaatsingsmethoden in de API voor middelen moeten de volgende aanvullende headers worden doorgegeven aan [!DNL Experience Manager]:

* X-bestemming
* X-diepte
* X-Overschrijven

[!DNL Experience Manager] -desktop maakt verbinding met [!DNL Experience Manager] een URL die de standaardpoort bevat. Daarom zou het plaatsen in de `virtualhosts` dispatcherconfiguratie het standaardhavenaantal moeten omvatten. Voor meer informatie over `virtualhosts` configuratie, zie virtuele gastheren [](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#identifying-virtual-hosts-virtualhosts)identificeren.
...
Als u de instellingen van de proxyserver voor Windows configureert of wijzigt (Internet-opties > LAN-instellingen), start u de [!DNL Experience Manager] bureaubladtoepassing opnieuw om de wijzigingen van kracht te laten worden.
...
In Windows bevindt de cache zich op `%LOCALAPPDATA%\Adobe\AssetsCompanion\Cache\`, waar een gecodeerde versie van de [!DNL Experience Manager] host is geconfigureerd in de bureaubladtoepassing. Bijvoorbeeld, `http://localhost:4502` verschijnt als `http%3A%2F%2Flocalhost%3A4502%2F`.
...
>[!NOTE]
>
>Als u de cache wist, blijven de niet-opgeslagen wijzigingen behouden. Elementen die niet op de [!DNL Experience Manager] server zijn ingecheckt, blijven behouden en worden niet verwijderd.
